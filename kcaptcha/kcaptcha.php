...
class KCAPTCHA{
	...
	function __construct($keystring){ // new PHP declaration
		...
		do{
			...
			for($i=0;$i<$length;$i++){
				...
				array_push($bbox, array($x-$shift, max($y, 0)), array($x-$shift+$m['end']-$m['start'], $y+$fontfile_height));
		...
		for($x=0;$x<$width;$x++){
			for($y=0;$y<$height;$y++){
				...
				for($bi=0; $bi<count($bbox); $bi++){
					if(abs(floor($sx) - $bbox[$bi][0]) <= 2 && abs(floor($sy) - $bbox[$bi][1]) <= 20){
						if (!array_key_exists($bi, $bbox_s)) {
							$bbox_s[$bi] = array($x, $y);
						} else {
							if ($bi % 2 == 0) { // start
								$bbox_s[$bi][0] = min($x, $bbox_s[$bi][0]);
								$bbox_s[$bi][1] = min($y, $bbox_s[$bi][1]);
							} else { // end
								$bbox_s[$bi][0] = max($x, $bbox_s[$bi][0]);
								$bbox_s[$bi][1] = max($y, $bbox_s[$bi][1]);
							}
						}
		...
		header("Captcha-String: $this->keystring");

		$bboxstr_2d = array();
		// for($bi=0; $bi<count($bbox); $bi++) {
		// 	$bboxstr_2d[] = implode(',', $bbox[$bi]);
		// }
		for($bi=0; $bi<count($bbox_s); $bi++) {
			$bboxstr_2d[] = implode(',', $bbox_s[$bi]);
		}
		$bboxstr_1d = implode('|', $bboxstr_2d);
		header("Captcha-Bbox: $bboxstr_1d");
		header("Content-Type: image/x-png");
		// imagepng($img);
		imagepng($img2);
